...
class JsonV0Test {
    ...
    fun `should create a json document from timesheet`() {
        ...
        val expected =
            ResourcesReader.stringFromResourceFile("${this.javaClass.packageName}/timesheet_v0_20220101-20221231.json")

        val actual =
            JsonV0(mockk(relaxed = true), objectMapper).create(emptyMap(), timesheet)

        assertSoftly { softly ->
            softly.assertThat(actual.exportType).isEqualTo(ExportType.JSON_V0)
            softly.assertThat(actual.fileName).isEqualTo("timesheet_20220101-20221231.json")
